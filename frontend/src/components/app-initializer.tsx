...
export function AppInitializer({ children }: { children: React.ReactNode }) {
  ...
  const checkPrerequisites = async (forceSuccess = false) => {
    setStatus((prev) => ({ ...prev, checking: true, error: undefined }))

    try {
      if (forceSuccess) {
        // Skip the actual check and force success
        setStatus({
          git: true,
          ssh: true,
          checking: false,
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/40 p-4">
      <Card className="w-full max-w-md">
        ...
        {!status.checking && (!status.git || !status.ssh) && (
          <CardFooter>
            <Button onClick={() => checkPrerequisites(true)} className="w-full" disabled={status.checking}>
              {status.checking ? (
                <>
