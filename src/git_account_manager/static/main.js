...
async function handle_delete_account(id) {
    const confirmed = await ui.show_confirm_dialog('Are you sure you want to delete this account?');
    if (!confirmed) return;
...
async function handle_copy_ssh_command(id) {
    ...
    await ui.copy_to_clipboard(command, button, 'SSH-add command copied to clipboard');
}
...
async function handle_create_project(event) {
    ...
    try {
        ...
        const project_data = {
            ...
            account_id: parseInt(document.getElementById('account_select').value),
            name: document.getElementById('project_path').value.split('/').pop()
        };
...
async function handle_delete_project(project_id) {
    const confirmed = await ui.show_confirm_dialog('Are you sure you want to delete this project?');
    if (!confirmed) return;
...
async function handle_browse_directory() {
    try {
        ...
        if (error.name !== 'AbortError') {
            ui.show_toast('Failed to select directory: ' + error.message, 'danger');
        }
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('account_form').addEventListener('submit', handle_create_account);
    document.getElementById('project_form').addEventListener('submit', handle_create_project);
